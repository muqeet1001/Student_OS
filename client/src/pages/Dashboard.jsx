...
export default function Dashboard() {
  ...
  const journey = useApiResource('/journey');
  ...
  return <div className="bg-background text-on-surface min-h-dvh"><div className="max-w-7xl mx-auto px-5 md:px-8 pt-16 lg:pt-6 pb-10 space-y-4"><header className="flex flex-col md:flex-row md:items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">Placement command centre</p><h1 className="font-headline text-2xl md:text-3xl font-black tracking-tight mt-1">{greeting()}, {firstName}</h1><p className="text-sm text-on-surface-variant mt-1">{data.readiness.score >= 80 ? 'You are ready to focus on applications.' : `${weakest?.label ?? 'Your preparation'} is the best place to improve next.`}</p></div><Link to="/my-plan" className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-full bg-primary text-on-primary text-sm font-bold">Open my plan <span className="material-symbols-outlined text-base">arrow_forward</span></Link></header>{journey.data && !journey.data.journey.onboardingComplete && <section className="rounded-xl bg-inverse-surface text-white p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[0.18em] font-black opacity-65">Start here</p><h2 className="font-headline text-lg font-black mt-1">Build your placement baseline</h2><p className="text-sm opacity-75 mt-1">Choose a role, timeline and weekly goal so every recommendation has context.</p></div><Link to="/get-started" className="px-5 py-2.5 rounded-full bg-white text-inverse-surface text-sm font-bold text-center">Set up my plan</Link></section>}<div className="grid grid-cols-1 lg:grid-cols-3 gap-4"><div className="lg:col-span-2"><ReadinessCard readiness={data.readiness} targetRole={targetRole} prominent /></div><TargetRole targetRole={targetRole} availableRoles={data.availableRoles} onChange={handleRoleChange} /></div><div className="grid grid-cols-1 lg:grid-cols-3 gap-4"><TodayPlan plan={data.plan} /><div className="lg:col-span-2"><Section title="Three priorities" action={<Link to="/my-plan" className="text-xs font-bold text-primary hover:underline">See complete plan</Link>}>{priorities.length === 0 ? <p className="text-sm text-on-surface-variant">No preparation blockers remain. Focus on eligible opportunities.</p> : <ol className="grid grid-cols-1 md:grid-cols-3 gap-2">{priorities.map((item, index) => <li key={item.id}><Link to={item.action.to} className="h-full flex gap-3 p-4 rounded-xl bg-surface-container-low hover:bg-surface-container"><span className="w-7 h-7 rounded-full bg-primary text-on-primary flex items-center justify-center text-xs font-black shrink-0">{index + 1}</span><div><p className="text-sm font-bold leading-snug">{item.text}</p><span className="inline-flex items-center gap-1 text-xs font-bold text-primary mt-2">{item.action.label}<span className="material-symbols-outlined text-sm">chevron_right</span></span></div></Link></li>)}</ol>}</Section></div></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-4"><Section title="Eligible opportunities" action={<Link to="/opportunities" className="text-xs font-bold text-primary hover:underline">Eligibility centre</Link>}><JobsForYou /></Section><Section title="Needs your attention" action={<Link to="/updates" className="text-xs font-bold text-primary hover:underline">All updates</Link>}><NotificationList notifications={data.notifications.slice(0, 3)} /></Section></div></div></div>;
}
